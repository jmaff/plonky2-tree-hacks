use plonky2::{
    hash::{
        ...
        hashing::hash_n_to_hash_no_pad, poseidon::PoseidonPermutation,
    },
    plonk::{config::{PoseidonGoldilocksConfig, GenericConfig, AlgebraicHasher}, circuit_data::VerifierOnlyCircuitData}, field::{goldilocks_field::GoldilocksField, extension::Extendable},
};
...
pub fn get_circuit_fingerprint_poseidon_goldilocks(
    ...
    let all_components: Vec<GoldilocksField> = verifier_data.constants_sigmas_cap.0.iter().flat_map(|f| f.elements).chain(verifier_data.circuit_digest.elements).collect();
    let output = hash_n_to_hash_no_pad::<GoldilocksField, PoseidonPermutation<GoldilocksField>>(&all_components);
    output
}
...
pub fn get_w_circuit_fingerprint_poseidon_goldilocks(
    verifier_data: &VerifierOnlyCircuitData<PoseidonGoldilocksConfig, 2>,
) -> WHashOut<GoldilocksField>{
    WHashOut(get_circuit_fingerprint_poseidon_goldilocks(verifier_data))
}
#[cfg(test)]
mod tests {
    use plonky2::{
    ...
    pub fn compute_circuit_fingerprint_poseidon_goldilocks() {
        ...
        let w_fingerprint = get_w_circuit_fingerprint_poseidon_goldilocks(&data.verifier_only).to_string();
        assert_eq!(w_fingerprint, "1cda093d8ad955a582c3c08d2feab763907835305c1e29d0ab6dd23dca9fba44");
    }
}
