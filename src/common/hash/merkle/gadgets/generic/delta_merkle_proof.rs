...
impl<H: GenericHashTarget, Hasher: GenericCircuitMerkleHasher<H>> GenericDeltaMerkleProofVecGadget<H, Hasher> {
    pub fn add_virtual_to<F: RicherField + Extendable<D>, const D: usize>(
        ...
        let siblings: Vec<H> = (0..height)
            .map(|_| H::create_virtual(builder))
            .collect();
    ...
    pub fn set_witness<F: RicherField, HashValue: WitnessValueFor<H, F, BIG_ENDIAN>, const BIG_ENDIAN: bool>(
        &self,
